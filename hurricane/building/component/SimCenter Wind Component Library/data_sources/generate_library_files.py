...
def create_fragility_files():
    ...
    fragility_data = input_data[fragility_cols].set_index('ID').sort_index().convert_dtypes()

    fragility_data.to_csv("fragility.csv")

    # now open the metadata header file
    with open("fragility_header.json",'r') as f:
        meta = json.load(f)
    ...
    with open("fragility.json",'w') as f:
